...
func (g Group) Write(entry, format string, args ...interface{}) error {
	...
	if err != nil {
		return g.error("%q: failed to open: %v", entry, err)
	}
	defer f.Close()

	data := fmt.Sprintf(format, args...)
	if _, err := f.Write([]byte(data)); err != nil {
		return g.error("%q: failed to write %q: %v", entry, data, err)
	}
...
func (g Group) readPids(entry string) ([]string, error) {
	...
	if err != nil {
		return nil, g.error("failed to open %q: %v", entry, err)
	}
	defer f.Close()
	...
	if s.Err() != nil {
		return nil, g.error("failed to read %q: %v", entry, err)
	}
...
func (g Group) writePids(entry string, pids ...string) error {
	...
	if err != nil {
		return g.error("failed to write pids to %q: %v", pidFile, err)
	}
	defer f.Close()
	...
	for _, pid := range pids {
		if _, err := f.Write([]byte(pid)); err != nil {
			if !errors.Is(err, syscall.ESRCH) {
				return g.error("failed to write pid %s to %q: %v",
					pidFile, pid, err)
			}
		}
	}
...
func (g Group) error(format string, args ...interface{}) error {
	name := strings.TrimPrefix(string(g), mountDir+"/")
	return fmt.Errorf("cgroup "+name+": "+format, args)
}

// cgroupError returns a formatted cgroup-specific error.
func (g Group) cgroupError(format string, args ...interface{}) error {
	return fmt.Errorf("cgroup: "+format, args...)
}

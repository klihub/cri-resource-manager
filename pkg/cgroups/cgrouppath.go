	...
	for classIdx, class := range cgpaths {
		for fnIdx, fn := range podPathGenFns {
			for _, dir := range fn(UID, class) {
				if info, err := os.Stat(path.Join(cpusetDir, dir)); err == nil {
					if info.Mode().IsDir() {
...
// GetPodPathGenerators return the pod path generating functions (for testing).
func GetPodPathGenerators() []func(UID, class string) []string {
	fns := make([]func(string, string) []string, len(podPathGenFns))
	copy(fns, podPathGenFns)
	return fns
}
...
var podPathGenFns = []func(UID, class string) []string{
	...
	func(UID, class string) []string {
		...
		return []string{
			...
			path.Join("/", kubeDir, qosDir, podDir),
			// without --cgroups-per-qos
			path.Join("/", kubeDir, podDir),
		}
	},
	// cgroups driver
	func(UID, class string) []string {

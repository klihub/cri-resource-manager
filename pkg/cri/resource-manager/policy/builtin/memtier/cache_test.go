...
func TestAllocationMarshalling(t *testing.T) {
	tcases := []struct {
	...
	}{
		{
			...
			data: []byte(`{"key1":{"Exclusive":"1","Part":1,"Container":"1","Pool":"testnode","MemType":"DRAM,PMEM,HBMEM","Memset":"","MemoryLimit":0}}`),
		},
		{
			name: "zero Exclusive",
			data: []byte(`{"key1":{"Exclusive":"","Part":1,"Container":"1","Pool":"testnode","MemType":"DRAM,PMEM,HBMEM","Memset":"","MemoryLimit":0}}`),
		},
	}
	for _, tc := range tcases {
		t.Run(tc.name, func(t *testing.T) {

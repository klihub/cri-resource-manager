...
func RequestMessage(kind, name, qualif string, req interface{}, sync bool) {
	if !dump.disabled {
		var ch chan struct{}
		if sync {
			ch = make(chan struct{})
...
func ReplyMessage(kind, name, qualif string, rpl interface{}, latency time.Duration, sync bool) {
	if !dump.disabled {
		var ch chan struct{}
		if sync {
			ch = make(chan struct{})
	...
	go func() {
		for req := range d.q {
			if req.dir != nop {
				...
				switch detail {
				case Name:
					d.name(req.dir, req.kind, method, req.qualif, req.msg, req.latency)
				case Full:
					d.full(req.dir, req.kind, method, req.qualif, req.msg, req.latency)
				}
			}
			if req.sync != nil {
				close(req.sync)
...
func (d *dumper) name(dir direction, kind, method, qualif string, msg interface{}, latency time.Duration) {
	var hdr string
...
func (d *dumper) full(dir direction, kind, method, qualif string, msg interface{}, latency time.Duration) {
	var hdr string
